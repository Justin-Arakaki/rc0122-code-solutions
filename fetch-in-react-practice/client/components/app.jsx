...
export default class App extends React.Component {
  ...
  toggleCompleted(todoId) {
    ...
    for (let i = 0; i < this.state.todos.length; i++) {
      newTodos[i] = this.state.todos[i];
    }
    for (const x in newTodos) {
      if (newTodos[x].todoId === todoId) {
        status = { isCompleted: !newTodos[x].isCompleted };
        newTodos[x].isCompleted = !newTodos[x].isCompleted;
        break;
      }
    }
